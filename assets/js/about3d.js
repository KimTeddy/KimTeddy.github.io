...
(function () {
  ...
  function init() {
    ...
    scene.fog = new THREE.Fog(floorColor, 10, 40);
    ...
    controls.enableZoom = false; // Disable scroll-to-zoom (Fixes scroll blocking issue)
  ...
  function createRooms() {
    const rooms = [
      ...
      { name: '안방 (Master Room)', color: 0x44aaff, x: 5, z: 4.5, w: 6, d: 7 }
    ];
    ...
    const glassMaterial = new THREE.MeshBasicMaterial({ color: 0x88ccff, transparent: true, opacity: 0.15, side: THREE.DoubleSide });
    ...
    const doorEdges = new THREE.LineSegments(new THREE.EdgesGeometry(doorGeo), new THREE.LineBasicMaterial({ color: 0x88ccff, transparent: true, opacity: 0.3 }));
    ...
    const windowEdges = new THREE.LineSegments(new THREE.EdgesGeometry(windowGeo), new THREE.LineBasicMaterial({ color: 0x88ccff, transparent: true, opacity: 0.3 }));
    windowEdges.position.set(4, 1.5, -8);
    scene.add(windowEdges);

    // 3. Small Room Desk
    // Bottom wall of small room is now at z = -2
    const deskMaterial = new THREE.MeshBasicMaterial({ color: 0xff4444, transparent: true, opacity: 0.05, side: THREE.DoubleSide });
    const deskGeo = new THREE.BoxGeometry(4, 1, 1.5);
    const desk = new THREE.Mesh(deskGeo, deskMaterial);
    desk.position.set(4, 0.5, -2.75);
    scene.add(desk);

    const deskEdges = new THREE.LineSegments(new THREE.EdgesGeometry(deskGeo), new THREE.LineBasicMaterial({ color: 0xff4444, transparent: true, opacity: 0.3 }));
    deskEdges.position.set(4, 0.5, -2.75);
    scene.add(deskEdges);

    // 4. Living Room TV
    // Right wall of living room (x = 2)
    const tvMaterial = new THREE.MeshBasicMaterial({ color: 0x44aaff, transparent: true, opacity: 0.1 });
    const tvGeo = new THREE.BoxGeometry(0.2, 2, 4);
    const tv = new THREE.Mesh(tvGeo, tvMaterial);
    tv.position.set(1.8, 1.5, 3);
    scene.add(tv);

    const tvEdges = new THREE.LineSegments(new THREE.EdgesGeometry(tvGeo), new THREE.LineBasicMaterial({ color: 0x44aaff, transparent: true, opacity: 0.3 }));
    ...
    const cabinetMaterial = new THREE.MeshBasicMaterial({ color: 0xaa8844, transparent: true, opacity: 0.1 });
    ...
    const cabinetEdges = new THREE.LineSegments(new THREE.EdgesGeometry(cabinetGeo), new THREE.LineBasicMaterial({ color: 0xaa8844, transparent: true, opacity: 0.3 }));
  ...
  function createEcosystem(nodeData) {
    ...
    nodeData.forEach(data => {
      ...
      if (data.type === 'box') geometry = new THREE.BoxGeometry(data.size, data.size, data.size, 2, 2, 2);
        ...
        geometry = new THREE.SphereGeometry(data.size * 0.6, 24, 24);
      }
      ...
      if (data.type === 'nest_hub') {
        mesh.scale.set(0.3, 1.2, 1.6); // Vertically flattened/stretched to look like a display
        mesh.rotation.y = Math.PI / 2;
      }
      
      mesh.userData = { name: data.name, desc: data.desc, baseColor: data.color, type: data.type, link: data.link };
      
      scene.add(mesh);
      nodes.push(mesh);

      // Create connection line to server (if not server)
      if (data.id !== 'server') {
        const materialLine = new THREE.LineBasicMaterial({ color: data.color, transparent: true, opacity: 0.3 });
        const points = [];
        points.push(new THREE.Vector3(serverNode.pos[0], serverNode.pos[1], serverNode.pos[2]));
        // curve up slightly
        const midX = (serverNode.pos[0] + data.pos[0]) / 2;
        const midZ = (serverNode.pos[2] + data.pos[2]) / 2;
        const midY = Math.max(serverNode.pos[1], data.pos[1]) + 2;
