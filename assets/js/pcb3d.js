...
(function () {
  ...
  // ARMI PCB enhancement state (shared module)
  let armiState = null;
  ...
  function loadModel() {
    ...
    function onSuccess(gltf) {
      ...
      // Tilt like a smartphone on a display stand (60 degrees back)
      pcbModel.rotation.x = Math.PI / 3; 
      pcbModel.position.y += homeModelY;

      // Apply ARMI PCB material enhancements, LED detection, and via splitting
      if (typeof ArmiPcbEnhance !== 'undefined') {
        armiState = ArmiPcbEnhance.process(pcbModel);
      }
  ...
  function animate() {
    ...
    if (pcbModel) {
      ...
      for (let i = 0; i < stageSpots.length; i++) {
        ...
        // Update fake beam opacity: sequential smooth fade in!
        if (s.beamMat) {
          const fadeDuration = 0.5; // fade over 50% of the Phase 1 duration for a softer look
          const turnOnThreshold = i * ((1.0 - fadeDuration) / Math.max(stageSpots.length - 1, 1));
          let lightFade = Math.min(Math.max((turnOnProgress - turnOnThreshold) / fadeDuration, 0), 1);
          // Apply smoothstep easing to eliminate stepped/jerky transitions
          lightFade = lightFade * lightFade * (3 - 2 * lightFade);
          s.beamMat.opacity = s.baseOpacity * lightFade;
        }
      }

      // ── ARMI PCB LED Animation (shared module) ──
      if (armiState && typeof ArmiPcbEnhance !== 'undefined') {
        ArmiPcbEnhance.animate(armiState, time, introElapsed);
      }
    } // End of if (pcbModel)
